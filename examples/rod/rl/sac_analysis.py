...
def parse_log_file(log_path):
    ...
    data = {
        ...
        'q_loss': np.array([iteration_data[i]['q_loss'] for i in sorted_iterations]),
        'entropy': np.array([iteration_data[i]['entropy'] for i in sorted_iterations]),
        'alpha': np.array([iteration_data[i]['alpha'] for i in sorted_iterations]),
...
def plot_metrics(data, plot_rewards=True, plot_q_loss=True, plot_entropy=True,
                 plot_alpha=True, plot_std=True, save_path=None):
    """
    ...
    subplots_needed = sum([plot_rewards, plot_q_loss, plot_entropy, plot_alpha, plot_std])
    ...
    # Plot q_loss
    if plot_q_loss:
        axes[subplot_idx].plot(iterations, data['q_loss'], 'r-', linewidth=2, label='Q Loss')
        axes[subplot_idx].set_xlabel('Iteration')
        axes[subplot_idx].set_ylabel('Q Loss')
        axes[subplot_idx].set_title('Q Loss vs Iteration')
        axes[subplot_idx].grid(True, alpha=0.3)
        axes[subplot_idx].legend()
        subplot_idx += 1
    ...
    # Plot alpha
    if plot_alpha:
        axes[subplot_idx].plot(iterations, data['alpha'], 'm-', linewidth=2, label='Alpha')
        axes[subplot_idx].set_xlabel('Iteration')
        axes[subplot_idx].set_ylabel('Alpha')
        axes[subplot_idx].set_title('Alpha vs Iteration')
        axes[subplot_idx].grid(True, alpha=0.3)
        axes[subplot_idx].legend()
        subplot_idx += 1
...
def print_statistics(data):
    ...
    metrics = ['rewards', 'q_loss', 'entropy', 'alpha', 'std']
...
def main():
    ...
    plot_metrics(
        ...
        plot_q_loss=not args.no_vf_loss,
        plot_entropy=not args.no_entropy,
        plot_alpha=not args.no_kl,
