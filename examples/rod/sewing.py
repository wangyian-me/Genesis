...
def main():
    ...
    c1 = RobotController(scene, franka1, ef1, args, (x1, y, z), initial_q_dof=open_gap)
    ...
    for i in range(14):
        if i != 6 and i != 7:
            c1.control_robot(0, 0, dz=z_delta_lift)
            c2.control_robot(0, 0, dz=z_delta_lift)
        else:
            c1.control_robot(0, 0, dx=x_delta1)
            c2.control_robot(0, 0, dx=x_delta2)
        for i in range(30):
            scene.step()
            for cid, cam in enumerate(cameras):
                img = cam.render()[0]
                frames[cid].append(img)
        gs.logger.info("lifted up")
