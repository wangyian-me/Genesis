...
def evaluate_batch(env: Train_Env, traj_list: List[np.ndarray], ratio: float, per_comp_bound, l2_bound) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if env.requires_grad:
        ...
        trajs_ = env.gd_one_step(trajs, ratio=ratio)
        trajs = np.zeros((n_envs, n_steps, act_dim), dtype=np.float32)
...
def optimize_trajectory_v2(
    ...
    while it < max_iters:
        ...
        # 2. Sort the trajectories by reward and apply GD step to the top 10%
        n_top = max(1, int(0.1 * pop))
        sorted_indices = np.argsort(-all_rewards)
        top_indices = sorted_indices[:n_top]

        X_selected = [X[i] for i in top_indices]

        len_ratio_list = len(ratio)
        # augment the selected trajectories for len_ratio_list times
        X_augmented = X_selected * len_ratio_list
        ratio_augmented = []
        for r in ratio:
            ratio_augmented.extend([r] * n_top)
        n_augmented = len(X_augmented)

        n_chunks_new = (max(1, (n_augmented + batch_size - 1) // batch_size))

        # 3. Evaluate the augmented trajectories with GD step

        all_rewards_new = []
        all_updated_trajs = []
        for ci, start in enumerate(range(0, n_augmented, batch_size), 1):
            t_chunk = time.time()
            chunk = X_augmented[start:start + batch_size]
            trajs = []
            for x in chunk:
                x_arr = np.asarray(x, dtype=np.float32)
                # (batch_size, n_steps, act_dim)
                ...
                tr = project_deltas(tr, per_comp_bound, l2_bound)
                trajs.append(tr)
            rewards, updated_trajs = evaluate_batch(env, trajs, ratio_augmented[start], per_comp_bound, l2_bound)
            all_rewards_new.extend(rewards.tolist())
            all_updated_trajs.append(updated_trajs)
            print(f"  └─ [selected] chunk {ci:>2}/{n_chunks_new}: {len(chunk):>3} evals | ratio={ratio_augmented[start]} | t={time.time() - t_chunk:.3f}s")

        all_rewards_new = np.asarray(all_rewards_new, dtype=np.float32)
        assert len(all_rewards_new) == n_augmented, f"Expected {n_augmented} new reward, got {len(all_rewards_new)}"
        # combine the new rewards with the original rewards
        all_rewards = np.concatenate([all_rewards, all_rewards_new], axis=0)

        # (n_augmented, n_steps, act_dim)
        all_updated_trajs = np.concatenate(all_updated_trajs, axis=0)
        assert len(all_updated_trajs) == n_augmented, f"Expected {n_augmented} updated trajs, got {len(all_updated_trajs)}"
        all_updated_trajs = all_updated_trajs.reshape(n_augmented, n_steps * act_dim)
        # combine the updated trajs with the original trajs
        X = X + list(all_updated_trajs)
        assert len(all_rewards) == len(X), f"Combined rewards length {len(all_rewards)} != trajs length {len(X)}"
