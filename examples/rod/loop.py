...
def main():
    ...
    control_robot_abs(args, franka1, ef1, force, force, g_dof_use_force=True, x=x1, y=y1, z=z+z_delta)
    control_robot_abs(args, franka2, ef2, force, force, g_dof_use_force=True, x=x2, y=y2, z=z+z_delta)
    for i in range(80):
        scene.step()
        for cid, cam in enumerate(cameras):
            img = cam.render()[0]
    ...
    control_robot_abs(args, franka1, ef1, force, force, g_dof_use_force=True, x=x1, y=y1, z=z+z_delta, quat=tq1)
    for i in range(80):
        scene.step()
        for cid, cam in enumerate(cameras):
            img = cam.render()[0]
