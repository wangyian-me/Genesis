...
def main():
    ...
    if args.path is not None:
        cameras.append(scene.add_camera(
            ...
            lookat=(0.0, 0., 0), fov=30, GUI=False
        ))
        cameras.append(scene.add_camera(
            res=(600, 450), pos=(-1.5, 1.2, 1.), up=(0, 0, 1),
            lookat=(0.0, 0., 0), fov=30, GUI=False
        ))
    ...
    lf = franka2.get_link("left_finger")
    for gi in lf._geoms:
        gripper_geom_indices.append(gi.idx)
    rf = franka1.get_link("right_finger")
    for gi in rf._geoms:
        gripper_geom_indices.append(gi.idx)
    rf = franka2.get_link("right_finger")
    for gi in rf._geoms:
        gripper_geom_indices.append(gi.idx)
